...
\begin{document}
	...
	\begin{titlepage}
		...
		{\bf Tipo:} {Matem\'aticas aplicadas, computaci\'on}\\
		
		{\bf Modalidad:} {Individual}\\
		
		{\bf N\'umero de p\'aginas:} {47}\\
	...
	{\let\clearpage\relax
		...
		{\Large \textbf{Algoritmos para la resoluci\'on del problema combinatorio de empaquetamiento}}\\
		
		\chapter*{Resumen}
	}
	En el siguiente trabajo se van a estudiar distintos tipos de algoritmos heur\'isticos y metaheur\'isticos aplicados a los problemas de optimizaci\'on combinatoria. En particular, el problema para el cu\'al vamos a tratar de buscar una soluci\'on lo m\'as pr\'oxima posible a su soluci\'on \'optima es el problema del empaquetamiento. Para ello, comenzaremos dando una breve introducci\'on sobre la optimizaci\'on combinatoria y las dificultades que presenta, y expondremos y formularemos el problema del empaquetamiento que vamos a tratar. Posteriormente, presentaremos a los algoritmos voraces, los cuales nos ofrecen una manera no muy complicada de obtener unas primeras soluciones para las distintas instancias del problema. A continuaci\'on, daremos una implementaci\'on m\'as eficiente de estos algoritmos para, finalmente y con ayuda de todos los algoritmos implementados hasta entonces, presentar los algoritmos evolutivos.
	\\
	
	Todos los m\'etodos que se han explicado en este trabajo se han implementado usando el lenguaje Scala.
	
	\vfill
	
	\textbf{Palabras clave:}\\
	
	\textsc{optimizaci\'on combinatoria, funci\'on objetivo, espacio de b\'usqueda, soluci\'on \'optima, algoritmos heur\'isticos, algoritmos metaheur\'isticos, algoritmos voraces, algoritmos evolutivos, complejidad algor\'itmica, coste asint\'otico, problemas p, problemas np, problema del empaquetamiento, bpp, \'arboles avl, nodos, altura, rotaciones, individuo, poblaci\'on, funci\'on de selecci\'on, funci\'on de cruce, funci\'on de mutaci\'on, scala}
	
	\pagebreak
	
	{\let\clearpage\relax
		
		{\Large \textbf{Algorithms for the resolution of Bin Packing Problem}}\\
		
		\chapter*{Abstract}
	}
	
	In the following project we are going to study different types of heuristic and metaheuristic algorithms applied to combinatorial optimisation problems. In particular, the problem for which we will try to find a solution as close as possible to its optimal solution is the bin packing problem. To do so, we will give a brief introduction to combinatorial optimisation and the difficulties it presents, and we will show and formulate the bin packing problem we are going to deal with. Subsequently, we will present the greedy algorithms, which offer us a not very complicated way to obtain some first solutions for the different instances of the problem. Then, we will give a more efficient implementation of these algorithms and, finally, with the help of all the algorithms implemented so far, we will present the evolutionary algorithms.
	\\
	
	All the methods explained in this project have been implemented using the Scala language.
	...
	\textsc{combinatorial optimisation, objective function, search space, optimal solution, heuristic algorithms, metaheuristic algorithms, greedy algorithms, evolutionary algorithms, algorithmic complexity, asymptotic cost, p-problems, np-problems, packing problem, bpp, avl trees, nodes, height, rotations, individual, population, selection function, crossover function, mutation function, scala}
	
	
	\chapter{Optimizaci\'on combinatoria}
	...
	Formalmente, un problema de optimizaci\'on se puede describir \cite{Tutor} como una tupla $(D, X, f, R)$ donde:
	...
	La notaci\'on $O(f)$ tiene las siguientes propiedades \cite{apunteseficiencia}, cualesquiera que sean las funciones $f,g$ y $h$:
	...
	No obstante, tenemos que tener cuidado a la hora de comparar el coste asint\'otico de los algoritmos. As\'i, por ejemplo, el algoritmo de Karatsuba \cite{Karatsuba} es un procedimiento para multiplicar n\'umeros grandes de manera eficiente. Es decir, para n\'umeros suficientemente grandes, este algoritmo realiza la multiplicaci\'on de dos n\'umeros m\'as r\'apido que el algoritmo cl\'asico. En contrapartida, si eligi\'eramos dos n\'umeros peque\~{n}os el algoritmo cl\'asico ser\'ia m\'as eficiente que el de Karatsuba. Lo mismo ocurrir\'ia si comparamos el algoritmo de multiplicaci\'on cl\'asico con el algoritmo de Toom-Cook (que es una generalizaci\'on del algoritmo de Karatsuba) o el de Sch\"onhage-Strassen.
	\\
	...
	\begin{definicion}
		Una clase de complejidad es un conjunto de problemas que poseen la misma complejidad computacional \cite{teoriaCC}.
	\end{definicion}
	...
	Otro importante subconjunto de la clase \textit{NP} son los problemas \textit{NP-completos} \cite{npcompleto}.
	...
	Una clase m\'as general de problemas no restringida a los problemas de decisi\'on es la clase de complejidad \textit{NP-dif\'icil} (\textit{NP-hard}) \cite{nphard}.
	...
	Esta clase contiene a aquellos problemas que son, como m\'inimo, tan dif\'iciles como un problema de \textit{NP}. De esta forma, la clase de problemas \textit{NP-completo} puede definirse como la intersecci\'on entre las clases \textit{NP} y \textit{NP-dif\'icil}.
	\\
	...
	Existen gran cantidad de m\'etodos heur\'{\i}sticos, lo cual hace que sea complicado dar una clasificaci\'on \cite{heuristicosOP} de los mismos ya que, por ejemplo, muchos de ellos han sido dise\~{n}ados para resolver un problema concreto. No obstante, podemos dar una clasificaci\'on general donde ubicar a los algoritmos heur\'{\i}sticos m\'as conocidos:
	
	\begin{itemize}
		\item \textbf{M\'etodos de descomposici\'on.} El problema de partida se descompone en subproblemas m\'as sencillos de resolver, sin perder de vista que ambos pertenecen al mismo problema. 
		...
		\item \textbf{M\'etodos constructivos.} Son m\'etodos que construyen paso a paso una soluci\'on del problema. Normalmente son m\'etodos deterministas y suelen estar basados en la mejor elecci\'on en cada iteraci\'on. 
	...
	Cuando se resuelve un problema por m\'etodos heur\'isticos, como la optimalidad no est\'a garantizada, se debe medir la calidad de los resultados. Para ello existen diversos procedimientros, entre los cuales podr\'iamos destacar los siguientes \cite{aproxheumeta}:
	...
	El BPP tambi\'en puede considerarse como un caso especial del \textit{cutting stock problem}, cuyo origen est\'a asociado a la industria maderera \cite{cutting}: 
	\\
	...
	Frecuentemente, los algoritmos de tipo greedy y los problemas que pueden resolver, se caracterizan por la mayor\'ia de las siguientes caracter\'isticas \cite{Brassard}:
	...
	\noindent\fbox{
		\begin{minipage}{\textwidth}
			\begin{algorithmic}
				...
				\State{//greaterThanTarget es un m\'etodo de b\'usqueda binaria con el que, dado un array de cubos ordenados seg\'un sus capacidades restantes de forma creciente y un elemento, encuentra la posici\'on del menor elemento del array que es mayor o igual que el elemento dado}
				\State{solution = $\emptyset$}\
				\For{(item $\leftarrow$ items)}
	...
	\begin{figure}[h]
		\centering
		\includegraphics[scale = 0.6]{desequilibrios.png}
	...
	A continuaci\'on esquematizaremos las dos implementaciones de los m\'etodos delete anteriores:
	\begin{itemize}
		\item M\'etodo \textbf{delete} para Best Fit. Con este m\'etodo vamos recorriendo los nodos del \'arbol desde la ra\'iz hasta el nodo con el cubo en el que introduciremos un nuevo elemento, el cu\'al borraremos del \'arbol si lo encontramos. La manera de hacerlo es la siguiente: 
		\\
		
		Supongamos que la capacidad restante del cubo del nodo ra\'iz es mayor o igual que el peso del elemento que queremos introducir. Entonces, todos los cubos de los hijos izquierdos tendr\'an una capacidad restante menor que la del nodo ra\'iz. Esto quiere decir que el cubo con la menor capacidad restante que es mayor o igual que el peso del elemento que queremos introducir se encuentra en los hijos izquierdos del nodo ra\'iz. Si la capacidad restante anterior hubiera sido menor, significar\'a que el nodo con el cubo que estamos buscando se encuentra en los hijos derecho. 
		\\
		
		El algoritmo que hemos implementado usa recursividad para aplicar esta idea, parando cuando hemos encontrado el nodo con el cubo que va a almacenar ese elemento, borr\'andolo del \'arbol y devolviendo el cubo del nodo borrado o cuando recorremos todo el \'arbol y no se ha encontrado ning\'un nodo en cuyo cubo podamos introducir el elemento.
		\item M\'etodo \textbf{delete} para Worst Fit. Este m\'etodo recuerda bastante al que se ha implementado en el algoritmo First Fit para \'arboles para introducir un nuevo nodo que almacene un nuevo cubo. Esto se debe a que, seg\'un el orden que hemos definido para los \'arboles que usan estos dos m\'etodos, el nodo cuyo cubo tienen la mayor capacidad restante es el que se encuentra al final de la espina derecha. Por lo tanto, el m\'etodo delete para Worst Fit, lo que hace es irse al final de la espipna derecha y comprobar si el elemento que queremos introducir cabe o no en el cubo del nodo. Si cabe, borra el nodo y devuelve el cubo que se encontraba en el nodo.
	\end{itemize}
	
	\subsection{Algoritmos evolutivos}
	Los algoritmos evolutivos son una serie de algoritmos metaheur\'isticos inspirados en el proceso evolutivo \cite{series/ncs/EibenS03}. Por lo tanto, la idea detr\'as de ellos es sencilla de comprender: partiendo de una poblaci\'on conocida, obtenemos mediante reproducci\'on y selecci\'on natural un individuo "mejor". A continuaci\'on, este nuevo individuo se incorpora a la poblaci\'on de partida y seguimos obteniendo nuevos individuos mediante los mecanismos anteriores y as\'i, sucesivamente. Evidentemente estos nuevos individuos no tienen por qu\'e ser necesariamente mejores, aunque evidentemente trataremos que as\'i sea. Posteriormente, tendremos que especificar cu\'ales son estos mecanismos que simulan los procesos de reproducci\'on y selecci\'on.
	\\
	
	De esta forma, el algoritmo evolutivo funcionar\'ia como describimos a continuaci\'on \cite{Vigo}:
	...
	\noindent\fbox{
		\begin{minipage}{\textwidth}
			\begin{algorithmic}
				...
				\State{//La condici\'on de parada del bucle while es que alcancemos la SO o que se agote el}
				\State{//tiempo.}
	...
	L\'ogicamente este es el primer fichero que hemos tenido que implementar. Con \'el hemos creado un objeto con el que representar la informaci\'on que guarda un cubo del problema as\'i como los m\'etodos principales para interactuar con el mismo.
	\\
	
	En primer lugar tenemos dos \'unicos atributos, a saber: \textit{items}, que es un array de tipo buffer y es la variable donde almacenaremos los elementos que se vayan introduciendo en el cubo y \textit{leftCapacity}, que almacena un entero,  que la clase recibe como argumento del constructor, y no es m\'as que la capacidad restante del cubo. El motivo por el que se elige el ArrayBuffer para almacenar los objetos que se van introduciendo es que es una estructura de datos mutable (que es lo que nos permite ir introduciendo nuevos elemento en el cubo) a la cual se le puede a\~{n}adir elementos de forma incremental eficientemente (con complejidad amortizada $O(1)$). 
	\\
	
	Por otra parte, podemos encontrar los siguientes m\'etodos principales:
	\begin{itemize}
		\item \textbf{add}: Este m\'etodo toma como par\'ametro un entero, correspondiente al peso de un objeto, y lo introduce en el cubo.
		\item \textbf{canAdd}: Dado un entero, esta funci\'on comprueba si el elemento se puede introducir en el cubo.
		\item \textbf{getLeftCapacity}: Con este m\'etodo podemos consultar la capacidad restante de la instancia del cubo en que estemos interesados.
	\end{itemize}
	...
	\begin{itemize}
		...
		\item \textbf{length}: M\'etodo de la clase Solution con el que devolvemos la longitud de la soluci\'on (n\'umero de cubos) de una instancia del problema.
	\end{itemize}
	...
	\begin{itemize}
		\item \textbf{greaterThanTarget}: Se trata de un m\'etodo de b\'usqueda binaria \cite{greaterthantarget} con el que, dado un entero y un ArrayBuffer de cubos ordenados seg\'un sus capacidades restantes de forma creciente, encuentra la posici\'on del cubo con la menor capacidad restante que es mayor o igual que el entero dado. 
		...
		\item \textbf{sortDescending}: Este es un m\'etodo de ordenaci\'on que usamos para ordenar de manera descendente los elementos de la lista de objetos que nos proporciona las distintas instancias del problema. B\'asicamente hacemos uso del algoritmo QuickSort \cite{sortDescending} para realizar tal ordenaci\'on.
	\end{itemize}
	...
	Como ya mencionamos en el cap\'itulo anterior, los m\'etodos necesarios para implementar los algoritmos Best Fit y Worst Fit mediante \'arboles AVL son pr\'acticamente iguales, cambiando \'unicamente el m\'etodo \textbf{delete} de ambos. Por tanto, para tener un c\'odigo m\'as limpio, se ha creado la clase abstracta AVLTree para implementar todos los m\'etodos de los que hacen uso ambos algoritmos. Posteriormente, las clases BFAVLTree y WFAVLTree heredan de la clase abstracta anterior para implementar, cada una de ellas, la versi\'on del m\'etodo delete que corresponda.
	\\
	...
	El fichero Population.scala consta de una clase abstracta Population que implementa los m\'etodos necesarios a la hora de interactuar con los distintos tipos de poblaciones. Los m\'as importantes son:
	\begin{enumerate}
		\item \textbf{binaryTournament}: M\'etodo que implementa el algoritmo del torneo binario con el cual seleccionamos un individuo de la poblaci\'on aleatoriamente. En general, el m\'etodo de selecci\'on por torneo escoge aleatoriamente \textit{k} individuos de la poblaci\'on y selecciona el mejor de ellos. En nuestro caso, el m\'etodo selecciona dos individuos de la poblaci\'on y devuelve el que minimiza la funci\'on objetivo, es decir, el que tiene un menor n\'umero de cubos.
		\item \textbf{binaryInsertion}: M\'etodo de inserci\'on binaria con el que, dado un nuevo individuo, sustituimos a uno de los que se encuentran en la poblaci\'on para reemplazarlo.
	\end{enumerate}
	...
	\begin{enumerate}
		\item Algoritmo de cruce seg\'un el esquema PMX \cite{crossoverPMX} (Partially Mapped Crossover): \textbf{pmx}.
	...
	\end{enumerate}
	Estos algoritmos van a permitir combinar dos permutaciones para, a partir de ellas, generar nuevas permutaciones con parte de la informaci\'on de cada una. Para m\'as informaci\'on acerca de estos esquemas as\'i como del c\'odigo en el que nos hemos basado para desarrollar los algoritmos que usamos, consultar \cite{Vigo}.
	\\
	...
	Finalmente tenemos la clase \textbf{EvolutionaryAlgorithm} la cual, siguiendo el esquema descrito en el cap\'itulo anterior, hace uso de las clases que acabamos de describir y sus m\'etodos para implementar el m\'etodo \textbf{evolve}, el cual implementa la inicializaci\'on del algoritmo evolutivo y su ciclo de optimizaci\'on iterativo.
	
	\chapter{Conclusiones}
	La finalidad de este trabajo es la de, a trav\'es del estudio de un problema de optimizaci\'on combinatoria concreto, pasar del marco te\'orico de las matem\'aticas al pr\'actico. Para ello, hemos hecho uso de la potencia de c\'omputo que nos ofrecen los ordenadores, los cuales nos permiten dar mejores soluciones para este tipo de problemas a medida que la tecnolog\'ia avanza. Este hecho ha quedado reflejado, sobre todo, en el algoritmo evolutivo que finalmente hemos desarrollado, ya que cuanto m\'as potente sea la m\'aquina de la que disponemos, m\'as iteraciones podremos realizar en el mismo intervalo de tiempo.
	\\
	...
	A continuaci\'on, hemos conseguido una mejora significativa en las complejidades de los algoritmos anteriores haciendo uso de la estructura de \'arboles AVL para almacenar la soluci\'on de las instancias del problema. Esto implica, a su vez, que la implementaci\'on del algoritmo sea m\'as compleja.
	\\
	
	Una vez hecho todo este trabajo, hemos podido pasar propiamente a la implementaci\'on del algoritmo evolutivo, pues distintas variedades de \'este basan su funcionamiento en los distintos m\'etodos presentados previamente. De esta forma, vemos que podemos tener un algoritmo evolutivo diferente por cada uno de estos m\'etodos. Adem\'as, si tenemos en cuenta los diferentes algoritmos de selecci\'on, de cruce y de mutaci\'on que hay, vemos que podemos encontrar multitud de combinaciones de todos estos m\'etodos que dan distintos algoritmos evolutivos, lo cual pone de manifiesto la cantidad de opciones existentes a la hora de obtener una "buena" soluci\'on de un problema dado en un tiempo "aceptable". 
	\\ 
