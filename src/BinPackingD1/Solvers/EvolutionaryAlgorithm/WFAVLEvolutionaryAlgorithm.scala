...
import BinPackingD1.Solvers.AVLTreeAlgorithm.WFAVLTree
...
class WFAVLEvolutionaryAlgorithm(val popLength: Int, instance: ProblemInstance, val probCross: Double, val seed: Int, val maxTime: Double) {
  ...
  val algorithmPopulation = new population.WorstFitAVLTreePopulation(rnd, popSize, instance) //Save child in the array last position
  ...
  def evolve(): Unit = {
    ...
    while(timer.elapsedTime < maxTime) {
      ...
      val treeWF = new WFAVLTree(currentInstance)

      algorithmPopulation.individuals(popLength).bins = treeWF.solve().length // Finds the solution for child
