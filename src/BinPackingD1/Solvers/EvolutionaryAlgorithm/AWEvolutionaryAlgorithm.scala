...
import BinPackingD1.Solvers.GreedyAlgorithm.AlmostWorstFit
...
class AWEvolutionaryAlgorithm(val popLength: Int, instance: ProblemInstance, val probCross: Double, val seed: Int, val maxTime: Double) {
  ...
  val algorithmPopulation = new population.AlmostWorstFitPopulation(rnd, popSize, instance) //Save child in the array last position
  ...
  def evolve(): Unit = {
    ...
    while(timer.elapsedTime < maxTime) {
      ...
      val almostWorstFit = new AlmostWorstFit(currentInstance)

      algorithmPopulation.individuals(popLength).bins = almostWorstFit.solve().length // Finds the solution for child
