...
import BinPackingD1.Solvers.AVLTreeAlgorithm.FFAVLTree
...
class FFAVLEvolutionaryAlgorithm(val popLength: Int, instance: ProblemInstance, val probCross: Double, val seed: Int, val maxTime: Double) {
  ...
  val algorithmPopulation = new population.FirstFitAVLTreePopulation(rnd, popSize, instance) //Save child in the array last position
  ...
  def evolve(): Unit = {
    ...
    while(timer.elapsedTime < maxTime) {
      ...
      val treeFF = new FFAVLTree(currentInstance)

      algorithmPopulation.individuals(popLength).bins = treeFF.solve().length // Finds the solution for child
