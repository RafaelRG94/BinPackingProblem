...
import BinPackingD1.Solvers.AVLTreeAlgorithm.BFAVLTree
...
class BFAVLEvolutionaryAlgorithm(val popLength: Int, instance: ProblemInstance, val probCross: Double, val seed: Int, val maxTime: Double) {
  ...
  val algorithmPopulation = new population.BestFitAVLTreePopulation(rnd, popSize, instance) //Save child in the array last position
  ...
  def evolve(): Unit = {
    ...
    while(timer.elapsedTime < maxTime) {
      ...
      val treeBF = new BFAVLTree(currentInstance)

      algorithmPopulation.individuals(popLength).bins = treeBF.solve().length // Finds the solution for child
