...
import BinPackingD1.Bin.Bin

import scala.collection.mutable.ArrayBuffer

object Utils {

  def greaterThanTarget(target: Int, elements: ArrayBuffer[Bin]): Int = {
    var start = 0
  ...
  def reorderBufferArrays(position: Int, binArrayToBeSorted: ArrayBuffer[Bin]): Unit = {
    ...
    val newPosition = greaterThanTarget(auxBin.getLeftCapacity, binArrayToBeSorted)
